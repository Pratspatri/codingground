...
{									                                   // start from the the block m and traverse right until we find a suitable block (first fit memory 									      allocation)
   if (m == NULL) 
   {
      return NULL;
   }
...
{									                                   // get the block resulting from the left merge, then do a right merge from there
   struct mab *ltmerge = mergeleft(m);
   return mergeright(ltmerge);
}

struct mab *mergeleft(struct mab *m) 
{									                                   // The idea is that we start from the given block and we traverse the linked list to the left until 									      we either hit the front of the list or we've hit a									 block that's not free. Then we merge from that 									    block to our current block to make one contiguous 									       free block, and return it
    if (m == NULL) 
    {
        return NULL;
    }
...
struct mab *mergeright(struct mab *m) 
{
    ...
    while (curr) 
    {
	 ...
	 free(prev);
    										                              ...
    										                              // by the end of the loop curr is either the null at the end or a nonfree so we 										      update the block and update the prev and 											 next pointers
    m->size = totalsize;
    m->allocated = FALSE;
    m->next = curr;
    if (curr) 
...
struct mab *memsplit(struct mab *m, int size) 
{
    ...
    m->allocated = TRUE;
    if (size < oldsize) 
    {										                         // if the desired size is less than the available space then we split the block by 										      creating a new block
	  ...
	  newmab->prev = m;
	  return m;
    }										                         // otherwise, the entire old block is filled so we mark it as used and return it
    return m;
}
