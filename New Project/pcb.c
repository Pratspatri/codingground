...
struct pcb *startpcb(struct pcb *process) 
{
    ...
    int e,f,g,h;
    if (a == 0) 
    { 										                         // realtime processes have priority 0 and no resources
	    e = 0;
	    f = 0;
	    g = 0;
	    h = 0;
...
struct pcb *enqpcb(struct pcb **head, struct pcb *process) 
{
    ...
    if (*head == NULL) 
    {
	    *head = process; 							                    // if the head is empty, set head to be the process
            return *head;
    }										                         // else, traverse the linked list until we come to the end then we queue the given 										      process onto the end
...
struct pcb *deqpcb(struct pcb **head) 
{
    ...
    if (*head == NULL) 
    { 										                         // if there's nothing at the head, we can't dequeue anything
	    return NULL;
...
void printq(struct pcb **head) 
{
    ...
    if (!*head) 
    { 										                         // there's nothing at the head, so the list is empty
	    return;
    }
    struct pcb *curr = *head;
    while (curr->next) 
    {										                         // traverse the linked list, printing out various process fields for debugging
            printf("ID: %d, arr time: %d, mb: %d\n", curr->pid, curr->arrivaltime, curr->mb);
	    curr = curr->next;
    }
    printf("ID: %d, arr time: %d, mb: %d\n", curr->pid, curr->arrivaltime, curr->mb);
}