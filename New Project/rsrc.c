...
struct rsrcb *rsrcchk(struct rsrcb *host, struct rsrcb *r) 
{
     ...
     { 										                         // compare the fields of each resource
     ...
     {										                         // this means that for each of the fields, the host's count was greater than the 										      required count, so we're able to allocate 										 the resource
	  return r;
     }
     return NULL;
}
...
void rsrcfree(struct rsrcb *host, struct rsrcb *r) 
{
     ...
     if (!r->allocated) 
     { 										                         // we don't need to free a resource that's not allocated
	  return;
     }
     int i;
     for (i=0; i<NUMRSRCTYPES; ++i) 
