...
void getjobs(struct pcb **q, char *filename) 
{
 ...
 { 										                              // read a line from input
    int arrtime, priority, cputime, mb, printer, scanner, modem, cd;
    int ret = sscanf(line,"%d, %d, %d, %d, %d, %d, %d, %d\n",&arrtime,&priority,&cputime,&mb,&printer,&scanner,&modem,&cd);
    if (ret != 8) 
    { 										                         // if the wrong number of fields is read
	exitwithmsg("hostd: error in input file format\n");
    }
    struct rsrcb *res;
    if (priority != 0) 
    { 										                         // if it's a user job, allocate its processes
...
{										                              // counts the number of non-empty round robin queues
   ...
   {
      if (*q_arr[i]) 
      { 									                              // if the head of the queue exists, update the count
         ++count;
      }
   }
   return count;
   ...
   *curr_proc = NULL; 								                    // initially, no currently running process
   ...
   {										                              // unload pending processes from input 											   queue
     while (*input_head && (*input_head)->arrivaltime <= disp_timer) 
     {
           ...
           if (temp->priority == 0) 
	   { 									                         // priority 0 processes are realtime
	         if (memchk(realtimeblk, temp->mb)) 
		 {
		    temp->procmem = memalloc(realtimeblk, temp->mb);
		 }
     ...
     while (*user_queue && memchk(memblock,(*user_queue)->mb) && rsrcchk(host,(*user_queue)->resources)) 
     {
       struct pcb *temp = deqpcb(user_queue); 					          // dequeue the topmost user job
       temp->procmem = memalloc(memblock, temp->mb); 				          // allocated memory
     ...
     { 										                         // if there's a process currently running
       ...
       if ((*curr_proc)->remainingcputime <= 0) 
       { 									                              // time's up
						             			                         // terminate the process and free the associated memory and also free the 											      process's allocated memory and resources
	...
	{
	     if (suspendpcb(*curr_proc)) 
	     { 									                         // suspend current process
                if ((*curr_proc)->priority < NO_RRQS) 
		{ 								                              // if possible, decrement the priority 											   (higher value => lower priority)
 	           (*curr_proc)->priority++;
	        }
     ...
     if (!*curr_proc && (*realtimeq || count_nonempty(rrqs) > 0)) 
     {
       ...
       if (*realtimeq) 
       { 									                              // if there's a realtime job, run it
             proc = deqpcb(realtimeq);
       }
       else 
       { 									                              // else find the highest priority user job 										   and run that
         i = 0;
         while (!proc) 
	 {
	     proc = deqpcb(rrqs[i]);
       ...
       { 									                              // if the process is new, start it
             startpcb(proc);
       }
       else 
       {
